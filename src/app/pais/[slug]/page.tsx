...
export default function CountryPage() {
  ...
  const stats = useMemo(() => {
    const available = countryProducts.filter(
      (product) =>
        product.available !== false &&
        product.visible !== false &&
        getAvailableStock(product) > 0
    ...
    const deals = countryProducts.filter((product) =>
      Boolean(product.salePrice)
    ).length;

    const wholesale = countryProducts.filter(
      (product) => product.wholesale
    ).length;

    const featured = countryProducts.filter(
      (product) => product.featured
    ).length;

    return {
      total: countryProducts.length,
      available,
      deals,
      wholesale,
      featured,
  ...
  return (
    <main className="min-h-screen bg-[#f6f8fc] text-slate-950">
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 md:px-6 md:py-10">
          ...
          <div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_0.42fr] lg:items-center">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-sm font-semibold leading-7 text-slate-300 md:text-base">
                {country?.description ??
                  "Productos disponibles según país de importación en RCA IMPORT. Explora modelos, accesorios y productos importados con disponibilidad actualizada."}
              </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <HeroStat title="Mayorista" value={stats.wholesale} />
            </div>
          </div>
        </section>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_280px_160px]">
            <div className="flex h-14 items-center gap-3 rounded-2xl border border-slate-200 bg-[#f6f8fc] px-4 transition focus-within:border-[#0057A8] focus-within:bg-white">
              ...
              <input
                ...
                placeholder={`Buscar productos de ${countryName}...`}
        ...
        <section id="productos-pais" className="mt-8">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-2 text-sm font-semibold text-slate-500">
                País de importación: {countryFlag} {countryName}
              </p>
            </div>
            ...
            <div className="inline-flex w-fit items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-black text-slate-600 shadow-sm">
              <Star size={18} className="text-[#0057A8]" />
          ...
          {loading ? (
            ...
            <div className="mt-6 overflow-hidden rounded-[2.5rem] border border-slate-200 bg-white shadow-sm">
              <div className="bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_35%),radial-gradient(circle_at_bottom_right,#fee2e2,transparent_35%)] p-10 text-center">
                <Globe2 className="mx-auto mb-4 text-slate-400" size={54} />

                <p className="text-2xl font-black">
                  Aún no hay productos de este país.
                </p>
                ...
                <p className="mx-auto mt-2 max-w-lg text-sm font-semibold leading-7 text-slate-500">
                  ...
                  estás buscando productos importados de {countryName}.
                </p>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <FeatureCard
            icon={Globe2}
            title="Origen filtrado"
            text="Esta vista muestra productos asociados al país de importación seleccionado."
          />

          <FeatureCard
            icon={PackageSearch}
            title="Consulta disponibilidad"
            text="Puedes revisar cada producto y consultar stock directo por WhatsApp."
          />

          <FeatureCard
            icon={Truck}
            title="Envíos coordinados"
            text="Los productos importados se coordinan para recojo o envío según disponibilidad."
          />
        </section>
      </section>
