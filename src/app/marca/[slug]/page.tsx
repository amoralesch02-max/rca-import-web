...
export default function BrandPage() {
  ...
  const stats = useMemo(() => {
    const available = brandProducts.filter(
      (product) =>
        product.available !== false &&
        product.visible !== false &&
        getAvailableStock(product) > 0
    ...
    const deals = brandProducts.filter((product) =>
      Boolean(product.salePrice)
    ).length;

    const wholesale = brandProducts.filter(
      (product) => product.wholesale
    ).length;

    const featured = brandProducts.filter(
    ...
    <main className="min-h-screen bg-[#f6f8fc] text-slate-950">
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 md:px-6 md:py-10">
        <div className="mb-6 flex flex-wrap items-center gap-2 text-sm font-bold text-slate-500">
          ...
          <span className="text-slate-950">{brandName}</span>
        </div>
          ...
          <div className="relative z-10 grid gap-8 lg:grid-cols-[1fr_0.42fr] lg:items-center">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-sm font-semibold leading-7 text-slate-300 md:text-base">
                {brand?.description ??
                  "Productos disponibles de esta marca en RCA IMPORT. Explora modelos, accesorios, ofertas y disponibilidad actualizada."}
              </p>
          ...
          <div className="grid gap-4 lg:grid-cols-[1fr_280px_160px]">
            <div className="flex h-14 items-center gap-3 rounded-2xl border border-slate-200 bg-[#f6f8fc] px-4 transition focus-within:border-[#0057A8] focus-within:bg-white">
              ...
              <input
                ...
                placeholder={`Buscar dentro de ${brandName}...`}
        ...
        <section id="productos-marca" className="mt-8">
          ...
          {loading ? (
            ...
            <div className="mt-6 overflow-hidden rounded-[2.5rem] border border-slate-200 bg-white shadow-sm">
              <div className="bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_35%),radial-gradient(circle_at_bottom_right,#fee2e2,transparent_35%)] p-10 text-center">
                <Tag className="mx-auto mb-4 text-slate-400" size={54} />

                <p className="text-2xl font-black">
                  Aún no hay productos de esta marca.
                </p>
                ...
                <p className="mx-auto mt-2 max-w-lg text-sm font-semibold leading-7 text-slate-500">
                  ...
                  estás buscando un producto específico de {brandName}.
                </p>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          <FeatureCard
            icon={BadgeCheck}
            title="Marca filtrada"
            text="Esta vista muestra únicamente productos asociados a la marca seleccionada."
          />

          <FeatureCard
            icon={PackageSearch}
            title="Consulta disponibilidad"
            text="Puedes revisar cada producto y consultar disponibilidad directa por WhatsApp."
          />

          <FeatureCard
            icon={ShoppingBag}
            title="Compra o separación"
            text="Agrega productos al carrito y registra una reserva con comprobante de pago."
          />
        </section>
      </section>
