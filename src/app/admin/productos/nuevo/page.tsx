...
function getFirstValidValue(currentValue: string, options: TaxonomyOption[]) {
  ...
  return options[0]?.name ?? currentValue;
}
...
export default function NewProductPage() {
  ...
  async function loadTaxonomies() {
    ...
    const [categoriesResult, brandsResult, countriesResult] =
      await Promise.all([
        supabase
          .from("categories")
          .select("name")
    ...
    setForm((currentForm) => {
      const nextCountryName = getFirstValidValue(
        currentForm.country,
        countries
      );

      const nextCountry = countries.find(
        (country) => country.name === nextCountryName
      );

      return {
        ...currentForm,
        category: getFirstValidValue(currentForm.category, categories),
        brand: getFirstValidValue(currentForm.brand, brands),
        country: nextCountryName,
        countryFlag: nextCountry?.flag || "🌎",
      };
    });
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (form.salePrice && (Number.isNaN(salePrice) || Number(salePrice) <= 0)) {
      setErrorMessage("Ingresa un precio de oferta válido o déjalo vacío.");
      return;
    }

    if (salePrice && salePrice >= normalPrice) {
    ...
    if (!result.success) {
      setErrorMessage(
        result.error?.includes("duplicate key")
          ? "Ya existe un producto con ese slug. Cambia el nombre o escribe un slug diferente."
          : "No se pudo crear el producto. Revisa los datos o permisos de Supabase."
      );
      return;
    }
  ...
  return (
    <AdminShell
      ...
      description="Registra productos reales en Supabase con imágenes, galería, video, stock y datos comerciales."
    >
      <Link
      ...
      >
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 p-6">
            ...
            <p className="mt-2 text-sm font-semibold text-slate-500">
              Selecciona categorías, marcas y países reales cargados desde
              Supabase.
            </p>
          ...
          <div className="grid gap-5 p-6">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <InputField
                ...
                placeholder="Ejemplo: Nuevo, Oferta, Destacado"
                icon={Star}
              />
        ...
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm xl:col-span-2">
          <div className="border-b border-slate-200 p-6">
            ...
            <p className="mt-2 text-sm font-semibold text-slate-500">
              Puedes subir archivos a Supabase Storage o pegar URLs externas.
            </p>
          </div>
          ...
          <div className="grid gap-5 p-6 lg:grid-cols-3">
            ...
            <section className="rounded-[1.7rem] border border-slate-200 bg-[#f6f8fc] p-5">
              ...
              <label className="mt-3 inline-flex h-14 w-full cursor-pointer items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 text-sm font-black text-white transition hover:bg-slate-800">
                <Upload size={18} />
                {uploadingGallery ? "Subiendo..." : "Subir galería"}
          ...
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
            <div>
              ...
              <h2 className="mt-4 text-3xl font-black">
                Guarda el producto para mostrarlo en el catálogo.
              </h2>

              <p className="mt-3 text-sm font-semibold leading-6 text-slate-300">
                Se guardará con categorías, marcas y países cargados desde tu
                base de datos.
              </p>
            </div>
